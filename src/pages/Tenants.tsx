...
const Tenants: React.FC = () => {
  ...
  const handleAmountChange = (value: string, inputType: "UGX" | "USD", field: "rentAmount" | "lastAmountPaid") => {
    ...
    const rate = exchangeRate || 3800;
    if (inputType === "UGX") {
      setNewTenant({ ...newTenant, [field]: numValue / rate });
    } else {
                ...
                viewHistory.paymentHistory.map((h: any, i: number) => (
                  <div key={i} className="flex justify-between items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <div className="text-right">
                      <p className="font-black text-emerald-600">+{Number(h.amount || 0).toLocaleString()} USD</p>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="relative col-span-2 md:col-span-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Rent (USD)</label>
                  <div className="relative mt-1">
                    <span className="absolute left-3 top-3.5 text-gray-400 text-xs font-bold">USD</span>
                    <input type="number" placeholder="Rent Amount" className="w-full pl-12 p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none font-bold" value={newTenant.rentAmount} onChange={(e) => handleAmountChange(e.target.value, "USD", "rentAmount")} />
                  </div>
                </div>

                <div className="relative col-span-2 md:col-span-1">
                  <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Rent (UGX Estimate)</label>
                  <div className="relative mt-1">
                    <span className="absolute left-3 top-3.5 text-gray-400 text-xs font-bold">UGX</span>
                    <input type="number" placeholder="UGX Amount" className="w-full pl-12 p-3 bg-blue-50/50 border border-blue-100 rounded-xl outline-none font-bold text-blue-700" value={(newTenant.rentAmount * (exchangeRate || 3800)).toFixed(0)} onChange={(e) => handleAmountChange(e.target.value, "UGX", "rentAmount")} />
                  </div>
                </div>
              </div>
